...
public class CustomerService {
    ...
    public void saveCustomerData() {
        String fileName = "customers.txt"; // Bạn có thể đổi thành đường dẫn đầy đủ
        try ( java.io.PrintWriter writer = new java.io.PrintWriter(new java.io.FileWriter(fileName))) {
            for (Customer c : customerList) {
                // Lưu định dạng: Mã|Tên|SĐT|Email
                writer.println(c.getCustomerCode() + "|"
                        + c.getName() + "|"
                        + c.getPhone() + "|"
                        + c.getEmail());
            }
            System.out.println("Customer data saved to " + fileName + " successfully!");
